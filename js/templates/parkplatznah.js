...
function oppPinAt(x, y, kind){
  ...
  return '<span class="opp-pin" data-rolle="' + (isP ? 'park' : 'hotel') + '" style="left:' + x + '%;top:' + y + '%">' +
    '<svg viewBox="-20 -42 40 42" width="30" height="31.5" aria-hidden="true">' + body + '</svg></span>';
}

/* ---------- Bild-Einpassung ----------
   Das eigene Kartenbild sitzt in einem Rahmen mit fester Proportion (wie der
   gezeichnete Plan) und lässt sich darin frei einpassen: bildZoom in Prozent
   (100 = eingepasst), bildX/bildY verschieben in Prozent der Rahmenbreite.
   Dieselbe Transformation nutzt auch das Ziehen in der Vorschau. */
function oppBildTransform(d){
...
function oppPlan(t, d, uid){
  ...
  if (useImg){
    ...
    return '<div class="opp-map opp-map--img">' +
      '<img src="' + esc(d.planBild) + '" alt="' + esc(t.mapTitle) + '" draggable="false"' +
        ' style="transform:' + oppBildTransform(d) + '">' +
      (pins
...
export default {
  ...
  fields:[
    ...
    { t:'note', label:'Einpassen geht direkt in der Vorschau: Bild ziehen zum Verschieben, Pins ziehen zum Platzieren, ＋/− oben zoomen. Die Zahlen hier sind für die Feinjustierung.' },
    { k:'bildZoom', label:'Bild-Zoom in %', type:'number', min:50, max:300, step:5,
  ...
  mount({ sheet, panel, state, save, repaint, rebuild }){
    panel.innerHTML = `
      <div class="vz-tools">
    ...
    const abwaerts = ev => {
      ...
      if (pinEl){
        ...
        zug = { art:'bild', rahmen, r, sx:ev.clientX, sy:ev.clientY,
    ...
    const bewegt = ev => {
      ...
      if (zug.art === 'pin'){
        ...
        else { state.hotelX = x; state.hotelY = y; }
        zug.el.style.left = x + '%'; zug.el.style.top = y + '%';
