/* Monatskalender · A4 hoch.
   --------------------------------------------------------------------------
   Das eine Blatt, das jedes Haus jeden Monat aufhängt: Abfuhrdaten,
   Reinigungstage, Anlässe, Zählerablesung. Bisher wurde dafür irgendein
   Kalender ausgedruckt und von Hand beschriftet — jetzt baut sich der Monat
   selbst: Wochen ab Montag, Kalenderwochen, Sonntage markiert, auf Wunsch
   die gesamtschweizerischen Feiertage (Ostern nach Gauss gerechnet).

   Die Einträge sind eine Liste: Tag + Text + Farbe. Was ausserhalb des
   Monats liegt, wird still ignoriert — so überlebt die Liste den
   Monatswechsel. */
...
export default {
  ...
  sub:'Der Monat baut sich selbst — Einträge, Feiertage, Kalenderwochen · A4 hoch',
  badge:'Aushang',
  page:'a4',
  root:'t-kal',
  ...
  fields:[
    ...
    { t:'note', label:'Tag ausserhalb des Monats? Der Eintrag bleibt gespeichert und erscheint einfach nicht.' },
    { k:'eintraege', label:'Einträge', type:'list', itemLabel:'Eintrag', max:20,
      defaultItem:{ tag:1, text:'', ton:'cyan' },
      item:[
  ...
  defaults:{
    ...
    eintraege:[
      { tag:3,  text:'Kehricht', ton:'cyan' },
      { tag:10, text:'Papier und Karton', ton:'gruen' },
      { tag:17, text:'Kehricht', ton:'cyan' },
      { tag:24, text:'Grünabfuhr', ton:'gruen' }
    ],
    fuss: contactLine()
  },
  ...
  render(d){
    ...
    const titel = has(d.titel) ? d.titel : `${KAL_MONATE[monat]} ${jahr}`;
